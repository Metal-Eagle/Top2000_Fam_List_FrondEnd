<template>
  <td>
    <img v-if="item.image" :src="item.image" class="img-thumbnail" />
    <svg
      v-if="!item.image"
      class="img-thumbnail"
      width="110"
  ...
  <td class="text-center">
    <div v-if="item.audio">
      <button
        @click="playSong(item.audio, item.id)"
        class="btn btn-outline-main"
      >
        <Play v-if="!playingSong.isPlaying || item.id !== playingSong.id" />
        <Pause v-if="playingSong.isPlaying && item.id === playingSong.id" />
      </button>
    </div>
  </td>
</template>
